...
3. **Navbar** — `dash_nav_title()`, refresh / help / logout buttons, user menu
...
else. `use_bs4Dashkit_core()` loads the CSS dependencies, activates the sidebar
...
Add hover-expand behaviour (sidebar expands on mouse-over even when collapsed):
...
- **Navigation utilities** — refresh, help, logout, user menu, and `dash_nav_title()`
