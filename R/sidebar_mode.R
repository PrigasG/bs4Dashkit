...
#'
#' @param brand_text   Visible brand label (character)
#' @param icon         Font Awesome icon name e.g. "shield-halved". NULL for none.
...
dash_brand_ui <- function(
  ...
  base_style_parts <- c(
    ...
    # solid color only when no gradient
  ...
  main_js <- sprintf("
    (function(){
      ...
      function init(){
        ...
        if(sidebar){
          sidebar.addEventListener('mouseenter', function(){ if(isCollapsed()) applyExpanded(); });
          sidebar.addEventListener('mouseleave', function(){ if(isCollapsed()) applyCollapsed(); });
        }
      }
