...
- Complete standardized navbar components (status, refresh, help,
  dividers, user menu)
...
For runnable examples, use the packaged topnav demo app:

``` r
app <- bs4dashkit_example_app(layout = "topnav")
shiny::runApp(app)
```

or the lower-level prototype:
...
``` r
dash_back_to_top(show_after = 400, position = "bottom-right")
```
