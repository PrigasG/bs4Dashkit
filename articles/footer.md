...
dash_footer() creates a
[`bs4Dash::bs4DashFooter()`](https://bs4dash.rinterface.com/reference/dashboardFooter.html)
with an optional logo image, a main text label (`left_text`), and a
right-side label (`right_text`, default: today’s date).

The logo and main text can be independently positioned on the left or
right side of the footer bar. The `right_text` is always rendered on the
right side.
...
Set `logo_src = NULL` to suppress the image entirely. This is the right
choice when you have no logo asset or want a plain, minimal footer. A
...
The bullet character (`\u2022`) is a compact separator commonly used in
footer text — e.g. `"NYS DOH \u2022 Internal Use Only"`.
...
is passed directly to the `footer` argument of
[`bs4DashPage()`](https://bs4dash.rinterface.com/reference/dashboardPage.html).
It should **not** be placed inside
...
The `logo_src` path is resolved relative to your app’s `www/` folder.
...
For apps bundled as a package (e.g. with
[`addResourcePath()`](https://rdrr.io/pkg/shiny/man/resourcePaths.html)),
use the path as exposed by the resource prefix, not the file-system
path.
