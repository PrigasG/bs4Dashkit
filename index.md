...
in your app; bs4Dashkit hides the sidebar and mirrors its menu into the
navbar so native tab switching still works.
...
keeps top-nav setup in one memorable object. The prefixed arguments such
as `topnav_align` and `topnav_gap` remain available for small one-off
overrides.
...
left or right tab alignment usually leaves the title clearer; centered
tabs are best when the navbar has no centered title or only a few tabs.
